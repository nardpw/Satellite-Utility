...
class ColorPick(Command):
    ...
    async def colorpick(self, message: discord.Message, args):
        try:
            ...
            colors = ImageUtils.extract_main_color('temp/image.png')
            ImageUtils.colors_to_image(colors, 'temp/image.png')
            await message.channel.send('Picked!', file=discord.File('temp/image.png'))
